...
void darDebaja(ePropetarios listado [],int limite,eCoches listacoche[],int TamCoches,eEgreso listaEgreso[])
{
    ...
    if(opcion!=-1)
    {
                for(i=0; i<TamCoches; i++)
            {
                ...
                {
                            horas=devolverHorasEstadia();
        total=tasarEstadia(listacoche[opcion].marca,horas);
        printf("Este usuario tiene :$%d\n",total);
        break;
        ...
        if(opcion2==1)
        {
           /* for(i=0; i<TamCoches; i++)
            ...
            egresos=buscarLugarLibreEgreso(listaEgreso,limite);
            listaEgreso[egresos].idEgreso=siguienteIdEgreso(listaEgreso,limite);
            listaEgreso[egresos].estado=EGRESADO;
            listaEgreso[egresos].importe=total;
           // listadoEgresos[egresos].marca=listaCoches[opcion].marca;
            //listaCoches[opcion].estado=BORRADO;
...
void ordenarDescendiente (ePropetarios ordenar[],int tamPropetarios)
{
   ePropetarios auxiliar;
    int i;
    int j;
    for(i=0;i<tamPropetarios-1;i++)
    {

       if(ordenar[i].estado==OCUPADO)
       {

            for(j=i+1;j<tamPropetarios;j++)
        {
     if(   strcmpi(ordenar[i].nombre,ordenar[j].nombre)<0)
     {
        auxiliar=ordenar[i];
        ordenar[i]=ordenar[j];
        ordenar[j]=auxiliar;
    ...
    mostrarPersonas(ordenar,tamPropetarios);
...
void pedirEdad(ePropetarios lista[],int tamPropetarios)
{
  ...
  int i;
   ...
   pedirMensaje("ingrese una edad para listar\n",auxiliar);
  validar_numero(auxiliar);
   edad=atoi(auxiliar);


        mostrarPersonasPoredad(lista,tamPropetarios,edad);
        getch();
...
/*int buscarPorEdad(ePropetarios listado[],int limite, int edad)
{
    ...
    for(i=0; i<limite; i++)
    {
        if(listado[i].estado == OCUPADO && listado[i].edad == edad)
        {
            retorno = i;
            break;
        }
