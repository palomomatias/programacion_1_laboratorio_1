...
void mostrarListaSeries(eSerie listado[], int largo)
{
    ...
    for(i=0;i<largo;i++)
    {
        ...
        {
            printf("\n\nNombre de la serie: %s Id de la serie: %d temporadas de la serie: %d genero de la serie: %s",listado[i].nombre,listado[i].idSerie,listado[i].cantidadTemporadas,listado[i].genero);
        }
    }
}
